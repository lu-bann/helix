...
use helix_beacon_client::{mock_block_broadcaster::MockBlockBroadcaster, mock_multi_beacon_client::MockMultiBeaconClient, BlockBroadcaster};
use helix_common::{chain_info::ChainInfo, signing::RelaySigningContext, Route, ValidatorPreferences};
...
use crate::{
    ...
    proposer::{
        ...
        PATH_GET_HEADER, PATH_GET_PAYLOAD, PATH_PROPOSER_API, PATH_REGISTER_VALIDATORS, PATH_STATUS,
    },
...
pub fn app() -> Router {
    ...
    Router::new()
        ...
        .route(&format!("{PATH_DATA_API}{PATH_VALIDATOR_REGISTRATION}"), get(DataApi::<MockDatabaseService>::validator_registration))
        .layer(Extension(api_service))
        .layer(Extension(data_api))
}

pub fn builder_api_app() -> (Router, Arc<BuilderApi<MockAuctioneer, MockDatabaseService, MockSimulator, MockGossiper>>, Receiver<Sender<ChainUpdate>>)
{
    ...
    let builder_api_service = Arc::new(BuilderApi::<MockAuctioneer, MockDatabaseService, MockSimulator, MockGossiper>::new(
        Arc::new(MockAuctioneer::default()),
        Arc::new(MockDatabaseService::default()),
        Arc::new(ChainInfo::for_mainnet()),
    ...
    let mut router = Router::new()
        .route(&Route::GetValidators.path(), get(BuilderApi::<MockAuctioneer, MockDatabaseService, MockSimulator, MockGossiper>::get_validators))
        .route(&Route::SubmitBlock.path(), post(BuilderApi::<MockAuctioneer, MockDatabaseService, MockSimulator, MockGossiper>::submit_block))
        .route(&Route::GetTopBid.path(), get(BuilderApi::<MockAuctioneer, MockDatabaseService, MockSimulator, MockGossiper>::get_top_bid))
    ...
    router = router.layer(
        ServiceBuilder::new()
            ...
            .layer(HandleErrorLayer::new(|err: BoxError| async move { (StatusCode::INTERNAL_SERVER_ERROR, format!("Unhandled error: {}", err)) }))
...
pub fn constraints_api_app() -> (Router, Arc<ConstraintsApi<MockAuctioneer>>, Receiver<Sender<ChainUpdate>>, Arc<MockAuctioneer>) {
    let (slot_update_sender, slot_update_receiver) = channel::<Sender<ChainUpdate>>(32);
    let auctioneer = Arc::new(MockAuctioneer::default());
    let constraints_api_service =
        Arc::new(ConstraintsApi::<MockAuctioneer>::new(auctioneer.clone(), Arc::new(ChainInfo::for_mainnet()), slot_update_sender.clone()));

    let router = Router::new()
        .route(&format!("{PATH_CONSTRAINTS_API}{PATH_GET_CONSTRAINTS}"), get(ConstraintsApi::<MockAuctioneer>::get_constraints))
        .route(&format!("{PATH_CONSTRAINTS_API}{PATH_GET_PRECONFER}"), get(ConstraintsApi::<MockAuctioneer>::get_preconfer))
        .route(&format!("{PATH_CONSTRAINTS_API}{PATH_GET_PRECONFERS}"), get(ConstraintsApi::<MockAuctioneer>::get_preconfers_for_epoch))
        .layer(RequestBodyLimitLayer::new(MAX_VAL_REGISTRATIONS_LENGTH))
