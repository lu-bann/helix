...
use helix_common::{chain_info::ChainInfo, signing::RelaySigningContext, BroadcasterConfig, NetworkConfig, RelayConfig};
...
impl ApiService {
    pub async fn run(mut config: RelayConfig) {
        ...
        let housekeeper = Housekeeper::new(db.clone(), multi_beacon_client.clone(), auctioneer.clone(), config.clone());
        ...
        let signing_key = SecretKey::try_from(signing_key_str).expect("could not convert env signing key to SecretKey");
        let public_key = signing_key.public_key();
        info!(relay_pub_key = ?public_key);
        let relay_signing_context = Arc::new(RelaySigningContext { signing_key, public_key, context: chain_info.context.clone() });

        let client = reqwest::ClientBuilder::new().timeout(SIMULATOR_REQUEST_TIMEOUT).build().unwrap();

        let simulator = OptimisticSimulator::<RedisCache, PostgresDatabaseService>::new(auctioneer.clone(), db.clone(), client, config.simulator.url);

        let (mut chain_event_updater, slot_update_sender) = ChainEventUpdater::new(db.clone(), chain_info.clone());
        ...
        let bids_cache: Arc<BidsCache> =
            Arc::new(Cache::builder().time_to_live(Duration::from_secs(10)).time_to_idle(Duration::from_secs(5)).build());

        let delivered_payloads_cache: Arc<DeliveredPayloadsCache> =
            Arc::new(Cache::builder().time_to_live(Duration::from_secs(10)).time_to_idle(Duration::from_secs(5)).build());

        let router =
            build_router(&mut config.router_config, builder_api, proposer_api, data_api, constraints_api, bids_cache, delivered_payloads_cache);
...
mod test {
    ...
    #[test]
    fn test() {
        let signing_key = SecretKey::try_from("0x123456789573772b8ffd9deddb468017a73cae08451ef05e604194705a1bade8".to_string())
